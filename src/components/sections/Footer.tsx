...
export const Footer: React.FC = () => {
  ...
  return <footer id="footer" className="pt-10 pb-16 lg:py-12 px-6 lg:px-12 bg-text flex flex-col lg:flex-row lg:justify-between lg:items-center w-full mt-10">
    <div className="flex flex-col lg:flex-row items-start lg:items-center space-y-8 lg:space-y-0 lg:space-x-10 text-[#9aa0a6] text-base font-medium flex-1">
      <div className="flex justify-between w-full lg:w-auto items-center mb-4 lg:mb-0">
        <a href="https://developers.google.com/" target="_blank" rel="noreferrer noopener" aria-label={t("google_developers_homepage")} onClick={() => analyticsService.trackNavigation('Google Developers', 'Footer', 'https://developers.google.com/')}>
          <img className="block" src="/images/Logo-GoogleForDevelopers.svg" aria-hidden="true" role="presentation" width="180" height="20" loading="lazy" alt="Google for Developers" />
        </a>
      </div>

      <div className="flex flex-wrap gap-x-6 gap-y-4">
        <Link href="/" className="hover:text-white transition-colors duration-200" onClick={() => analyticsService.trackNavigation('WOW 2026', 'Footer')}>{t("wow_2026")}</Link>
        <Link href="/terms" className="hover:text-white transition-colors duration-200" onClick={() => analyticsService.trackNavigation('Privacy & terms', 'Footer', '/terms')}>{t("privacy_terms")}</Link>
        <Link href="/code-of-conduct" className="hover:text-white transition-colors duration-200" onClick={() => analyticsService.trackNavigation('Community guidelines', 'Footer', '/code-of-conduct')}>{t("community_guidelines")}</Link>
        <Link href="/about" className="hover:text-white transition-colors duration-200" onClick={() => analyticsService.trackNavigation('FAQ', 'Footer', '/about')}>{t("faq")}</Link>
      </div>
    </div>
    ...
    <div className="flex flex-col lg:flex-row lg:items-center mt-10 lg:mt-0 gap-6 lg:gap-8">
      ...
      <div className="flex items-center gap-6 mt-4 lg:mt-0">
        <button className="text-[#9aa0a6] hover:text-white transition-colors duration-200 text-sm font-medium">{t("manage_cookies")}</button>

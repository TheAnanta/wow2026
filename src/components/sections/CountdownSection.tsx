...
function CountdownCard({
  ...
  return <div className="relative h-[90px] md:h-[184px]">
    {/* Front card */}
    <div className="absolute bg-grey-bg dark:bg-grey! w-full h-full rounded-xl border-[1.5px] md:border-2 border-solid border-grey dark:border-grey-bg z-10 overflow-hidden">
      <div className="flex flex-col items-center justify-center gap-y-1 h-full">
        <div className="flex flex-row justify-center gap-2 h-[50%] md:h-[90px]">
          {digits.map((digit, i) => <img key={i} src={`${BASE_IMAGE_URL}${digit}.svg`} aria-hidden="true" width="80" height="88" className="w-auto h-auto hcm-button" loading="lazy" />)}
        </div>
...
export function CountdownSection() {
  ...
  return <div className="h-countdown">
    <div aria-label={t("countdown_to_google_wow_2026")} aria-live="off" className="countdown-sr-announcer grid grid-cols-countdown-cards-2 md:grid-cols-countdown-cards-4 pb-[2.3rem] gap-y-8 gap-6">
      <span className="visually-hidden">{t("time_left_until_google_wow_2026")}{time.days}{t("days")}{time.hours}{t("hours")}{time.minutes}{t("minutes")}{time.seconds}{t("seconds")}</span>
      {units.map(unit => <CountdownCard key={unit.label} {...unit} />)}
    </div>
  </div>;
}
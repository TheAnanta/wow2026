...
export function Hero({
  ...
  const buttonText = !isRegistered ? t('register') : !hasTicket ? t('complete_registration') : t('update_profile');
  const buttonLink = !isRegistered ? '/register' : !hasTicket ? '/payment' : '/register';
  const handleCTAClick = () => {
    analyticsService.trackCTA(buttonText, 'Hero', 'click');
    router.push(buttonLink);
  };
  return <div className="flex flex-col items-center bg-grey-bg dark:bg-grey!">
      ...
      <div className="h-homepage-main-cta" data-bgimage="">
        <div className="flex flex-col items-center text-center p-6 max-w-[800px]">
          <h1 className="font-medium text-grey dark:text-white max-w-[360px] md:max-w-[500px] mb-4 sm:s-h3 md:l-h4">
            <span>
              <p>{t("tune_in_for")}<br />{t("gdg_on_campus_wow")}</p>
              <p>{t("july_4_2026")}</p>
            </span>
          </h1>
          ...
          <h2 className="font-medium text-grey dark:text-white mb-4 sm:s-h6 md:l-h6">
            <span>{t("participate_in_handson_workshops_tech_talks_and_hackathon_take_home_some_cool_swags_and_cash_prizes")}</span>
          </h2>
        ...
        <div className="hidden flex-col items-center">
          <h2 className="font-medium text-grey dark:text-white mb-4 sm:s-h6 md:l-h6">
            <span>{t("get_content_recommendations_by_updating_your_google_developer_profile")}</span>
          </h2>

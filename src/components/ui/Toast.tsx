...
export const Toast: React.FC<ToastProps> = ({ message, isVisible, onClose }) => {
  ...
  return (
    <div
      className={`fixed bottom-12 right-8 z-[9999] transition-all duration-300 ease-out ${
        isVisible ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0 px-4'
      }`}
    >
          ...
          <div className="h-full w-full z-10 flex flex-col">
            <div className="px-[4px] pt-[4px] w-full h-full">
              <div className="flex flex-col justify-center w-full h-full bg-white rounded-[15px] px-6 py-5">
                <button 
                  onClick={onClose}
                  className="absolute p-2 top-1 md:top-[12px] right-1 md:right-[12px] hover:bg-grey-bg rounded-full transition-colors" 

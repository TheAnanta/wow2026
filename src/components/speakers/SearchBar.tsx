...
export const SearchBar: React.FC<SearchBarProps> = ({
  ...
  return <div className="flex flex-col md:flex-row md:gap-7 mb-6">
    <div className="md:flex-1 w-full">
      <div className="flex relative w-full">
        <div className="flex items-center border-l-2 border-r-2 border-t-2 border-black dark:border-white dark:text-grey rounded-[100px] w-full h-[46px] md:h-14 bg-grey-bg dark:bg-white border-b-2">
          ...
          <input type="text" className="focus:outline-none focus:ring-0 border-0 bg-transparent p-0 text-lg font-medium w-full placeholder:text-black dark:placeholder:text-grey-600 placeholder-shown:text-black placeholder:text-ellipsis placeholder-shown:text-ellipsis" value={value} onChange={onChange} placeholder={placeholder} />
        </div>
      </div>
    </div>
  </div>;

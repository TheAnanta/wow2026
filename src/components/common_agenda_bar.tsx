...
const AgendaBar: React.FC<AgendaBarProps> = ({
  ...
  const { user } = useAuth() as any; // Use react-firebase-hooks wrapper
  ...
  return (
    <>
      ...
      >
        {/* Time */}
        <p className="absolute flex flex-col top-4 right-4 text-right">
          <b className="font-bold">{event.time.split(" to ")[0]}</b>
          <span className="text-sm">{event.time.split(" to ")[1]}</span>
        </p>
        {/* Title */}
        <h3 className="text-xl font-bold mb-2 pr-20 max-w-[220px] whitespace-pre-line max-h-14 overflow-hidden">
          {event.title}
        </h3>
        {/* Speakers */}
        <div
            ...
            eventSpeakers.slice(0, 3).map((speaker, index) => (
              ...
              >
                ...
                {eventSpeakers.length <= 3 && (
                  <p className="text-sm font-semibold overflow-hidden max-h-7 text-ellipsis line-clamp-2">
                    {speaker.name}
                  </p>
                )}
              </div>
      ...
      {isModalOpen && (
        <div
          className="fixed inset-0 z-50 flex items-end justify-center overflow-y-auto bg-black/50"
          onClick={closeModal}
        >
          <div
            className="relative w-screen max-w-screen bg-white p-6 shadow-lg transform transition-transform duration-300 translate-y-0"

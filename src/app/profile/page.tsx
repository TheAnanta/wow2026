...
function BadgeGrid({ badges }: { badges: any[] }) {
  ...
  return (
    <div className="w-full">
      <div className="p-6 bg-[#f5f2fa] dark:bg-[#1b1b21] rounded-2xl border border-[#c6c5d0] dark:border-[#46464f] transition-colors">
        <h2 className="text-2xl font-medium mb-8 tracking-tight text-[#1b1b21] dark:text-[#e5e1e9]">Badges earned</h2>
        {badges.length > 0 ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
            {badges.map((item, index) => (
              ...
              >
                ...
                <p className="text-sm font-medium mt-3 text-center line-clamp-1 tracking-tight dark:text-white">
                  {item.name}
                </p>
          ...
          <div className="text-center py-12 border-2 border-dashed border-grey-bg dark:border-white/10 rounded-2xl">
            ...
            <a href="/payment?tier=group" className="text-google-blue font-bold hover:underline">Get your first badge &rarr;</a>
          </div>
        )}
      </div>
...
function ProfileCard({
  ...
  return (
    <div className="w-full flex flex-col items-center">
      ...
      <div className="w-full bg-[#f5f2fa] dark:bg-[#1b1b21] border border-[#c6c5d0] dark:border-[#46464f] rounded-[32px] p-8 pt-24 relative transition-colors shadow-sm">
        <div
          className="absolute top-[-1px] left-1/2 -translate-x-1/2 w-48 h-24 border-x border-b transition-colors z-0 rounded-b-full shadow-sm bg-[#f5f2fa] dark:bg-[#1b1b21] border-[#c6c5d0] dark:border-[#46464f]"
          style={{ clipPath: 'inset(0 -10px -10px -10px)' }}
        />

        <div className="text-center mt-4">
          <h2 className="text-3xl font-bold tracking-tighter flex items-center justify-center gap-2 dark:text-white">
          ...
          {userDetails.username && (
            <a
              ...
              className="w-max mx-auto flex items-center gap-1 mt-3 border-2 border-grey-900 dark:border-white/20 text-grey-900 dark:text-white font-medium px-6 py-2 rounded-full text-sm no-underline hover:bg-gray-50 dark:hover:bg-white/10 transition-colors"
            >
              wow.vizag.dev/p/{userDetails.username}{" "}
              <svg xmlns="http://www.w3.org/2000/svg" height="18px" viewBox="0 -960 960 960" width="24px" fill="currentColor" className="text-google-blue"><path d="M200-120q-33 0-56.5-23.5T120-200v-560q0-33 23.5-56.5T200-840h280v80H200v560h560v-280h80v280q0 33-23.5 56.5T760-120H200Zm188-212-56-56 372-372H560v-80h280v280h-80v-144L388-332Z" /></svg>
            </a>
          )}

          <div className="text-left space-y-4 mb-8 mt-8 dark:text-white">
            {userDetails.speaker && (
              <>
                <p className="font-semibold mt-2 mb-1">Talk Title</p>{" "}
                <p className="opacity-80">{userDetails.speaker.talk || "N/a"}</p>
              </>
            )}
            {userDetails.city && userDetails.city !== "" && (
              <>
                <p className="font-semibold mt-2 mb-1">City/Town</p>{" "}
                <p className="opacity-80">{userDetails.city}</p>
              </>
            )}
            {userDetails.bio && userDetails.bio !== "" && (
              <>
                <p className="font-semibold mt-2 mb-1">Bio</p>{" "}
                <p className="opacity-80">{userDetails.bio}</p>
              </>
            )}
                  ...
                  <div className="flex flex-wrap">
                    {userDetails.interests.map(
                      (domain: any, index: any) => (
                        <span
                          key={index}
                          className="inline-flex items-center rounded-full bg-grey-900/10 dark:bg-white/10 px-3 min-h-8 text-sm m-1 tracking-tight font-medium"
                        >
                          {" "}
                          {domain}
                        </span>
            ...
            <p className="flex items-center gap-1 tracking-tight font-medium">
              <svg xmlns="http://www.w3.org/2000/svg" height="20px" viewBox="0 -960 960 960" width="20px" fill="currentColor"><path d="M371.01-324 480-390.22 589-324l-29-124 97-84-127-11-50-117-50 117-127 11 96.89 83.95L371.01-324ZM480-62.91 358.8-183.87H183.87V-358.8L62.91-480l120.96-121.2v-174.93H358.8L480-897.33l121.2 121.2h174.93v174.93L897.33-480l-121.2 121.2v174.93H601.2L480-62.91Zm0-117.31 86.65-86.65h126.48v-126.48L779.78-480l-86.65-86.65v-126.48H566.65L480-779.78l-86.65 86.65H266.87v126.48L180.22-480l86.65 86.65v126.48h126.48L480-180.22ZM480-480Z" /></svg>
      ...
      <main className="max-w-7xl mx-auto px-6 py-12">
        <div className="flex flex-col lg:flex-row gap-12 lg:items-start">
          ...
          <div className="flex-1 space-y-12">
            ...
            {/* Referral Card */}
            <ReferralCard />            {/* Tickets Section */}
            <div className="p-8 bg-[#f5f2fa] dark:bg-[#1b1b21] rounded-[32px] border border-[#c6c5d0] dark:border-[#46464f] transition-colors shadow-sm">
              ...
              {tickets.length > 0 ? (
                ...
                <div className="text-center py-12 border-2 border-dashed border-grey-bg dark:border-white/10 rounded-2xl">
                  ...
                  <a href="/payment?tier=group" className="text-google-blue font-bold hover:underline">Browse Tickets &rarr;</a>
                </div>
              )}
            </div>
              ...
              <div className="p-8 rounded-[32px] border-2 border-grey-900 dark:border-white/10 overflow-hidden relative group transition-colors"
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                  <div className="flex gap-4">
                    ...
                    <div>
                      <h3 className="text-2xl font-bold tracking-tight dark:text-white">Manage your Squad</h3>

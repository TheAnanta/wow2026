...
const GRADIENTS = ['linear-gradient(90deg, #4285F4 -36.98%, #4285F4 22.31%, #34A853 78.95%, #34A853 132.93%)', 'linear-gradient(270deg, #FFCB32 6.94%, #FFCB32 27.99%, #34A853 73.59%, #34A853 94.64%)', 'linear-gradient(90deg, #EA4335 -36.98%, #EA4335 22.31%, #4285F4 78.95%, #4285F4 132.93%)', 'linear-gradient(270deg, #34A853 6.94%, #34A853 27.99%, #FFCB32 73.59%, #FFCB32 94.64%)'];
export default function SpeakerDetailPage() {
  ...
  if (!speaker) {
    return <div className="w-full min-h-screen flex flex-col items-center justify-center">
                <h1 className="text-2xl font-medium mb-4">{t("speaker_not_found")}</h1>
                <Link href="/speakers" className="cta-primary">{t("back_to_speakers")}</Link>
            </div>;
  }
  return <div className="w-full flex flex-col min-h-screen">
                ...
                <div className="w-full flex flex-col md:flex-row text-md:h-[407px] bg-grey-bg dark:bg-grey! border-b md:border-b-2 border-grey dark:border-grey-bg">
                    <div className="flex flex-col md:text-left md:justify-center px-4 py-5 w-full md:w-3/5 md:p-10 md:pr-0 dark:text-white z-10">
                        ...
                        <div className="flex flex-row mt-2 md:mt-8">
                            ...
                            <Link className="ml-2 font-medium hover-link" href="/speakers" onClick={() => analyticsService.trackNavigation('All Speakers', 'SpeakerDetail_Hero')}>{t("see_all_speakers")}</Link>
                        </div>
                    </div>

                    <div className="flex justify-center md:justify-end items-center w-full md:w-2/5 px-4 py-5 md:p-10">
                        <img className="rounded-full w-48 max-w-md md:w-72 md:h-72 border-2 border-black object-cover" src={speaker.avatar ? `/images/speakers/${speaker.avatar}` : '/images/speakers/placeholder.webp'} role="img" aria-hidden="true" loading="lazy" alt={speaker.name} />
                    </div>
                </div>
                ...
                <div className="page-wrapper">
                    <section className="mt-2 dark:text-grey-200">
                        <div className="flex items-center justify-start md:justify-end mb-10">
                            <Link className="font-medium text-grey underline underline-offset-2 text-[14px] text-md:text-[20px] dark:text-white md:mt-auto" onClick={() => analyticsService.trackNavigation('All Sessions', 'SpeakerDetail_SubHeader')} href="/explore">{t("see_all_sessions")}</Link>
                        </div>
                    </section>

                    {/* Sessions Grid */}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10 md:mb-14">
                        {sessions.map((session, index) => <div key={session.id} className="group hover:border-[1.5px] border-grey dark:border-grey-bg rounded-xl flex flex-1">
                                <Link className="flex flex-col relative h-auto w-full" href={`/explore/${session.id}`} onClick={() => analyticsService.trackCTA(session.title, 'SpeakerDetail_Sessions')}>
                                    <div className="relative bg-white dark:bg-grey-900! w-full h-full rounded-xl group-hover:rounded-[10px] border-[1.5px] md:border-2 border-solid border-grey dark:border-grey-bg z-10 overflow-hidden flex-1">
                                        <div className="flex flex-col gap-y-1 text-md:gap-y-4 h-full p-8 md:p-10 z-2">
                                            ...
                                            <div className="card__keywords flex flex-wrap gap-3 justify-start mt-6">
                                                <p className="sr-only">{t("session_tags")}</p>
                                                {session.tags.map(tag => <span key={tag} className="bg-grey-bg dark:bg-white text-grey border-2 border-grey dark:border-grey-bg rounded py-1.5 px-4 text-sm">
                                                        <div>
                                                            <span>{tag}</span>
                                                        </div>
                                                    </span>)}
                                    ...
                                    <div className="relative mt-[-22px] md:mt-[-24px] rounded-xl group-hover:rounded-[10px] border-[1.5px] md:border-2 border-solid border-grey dark:border-grey-bg bg-cover h-[44px] md:h-[48px] z-1" style={{
                backgroundImage: GRADIENTS[index % GRADIENTS.length]
              }}></div>
                                    <div className="mt-[-22px] md:mt-[-24px] rounded-xl border-[1.5px] md:border-2 border-solid border-grey dark:border-grey-bg bg-cover h-[44px] md:h-[48px]" style={{
                backgroundImage: GRADIENTS[index % GRADIENTS.length]
              }}></div>
                                </Link>
                            </div>)}
                        {sessions.length === 0 && <div className="col-span-full py-10 text-center text-grey-600 dark:text-grey-400">
                                <p className="text-xl">{t("no_sessions_found_for_this_speaker")}</p>
                            </div>}
                    </div>
                        ...
                        <div className="flex flex-col align-center items-stretch justify-center mt-1 md:mt-10 gap-y-4 md:gap-x-8 md:flex-row ml:min-h-[400px]">
                            <div className="flex w-full md:w-1/2">
                                <div className="flex-1 flex flex-col overflow-hidden bg-grey-bg dark:bg-grey! border md:border-2 border-grey rounded-[16px] dark:border-white text-md:flex-row">
                                    <div className="promo-card__body flex-1 flex flex-col items-start p-6 ml:p-10 ml:pr-0">
                                        <div className="text-grey dark:text-white mb-3 text-md:mb-4 sm:s-h4 md:l-h4">{t("join_us_at_io_connect")}</div>
                                        <div className="flex-1 flex flex-col justify-between items-start">
                                            <p className="text-grey dark:text-white mb-3 text-md:mb-6 sm:s-cta2 md:l-cta1">{t("explore_network_and_get_handson_with_the_latest_products")}</p>
                                            <a href="https://developers.google.com/events" rel="noreferrer" target="_blank" className="cta-secondary" onClick={() => analyticsService.trackCTA('Find an event', 'SpeakerDetail_PromoConnect', 'external_link')}>{t("find_an_event")}</a>
                                        </div>
                                    </div>
                            ...
                            <div className="flex w-full md:w-1/2">
                                <div className="flex-1 flex flex-col overflow-hidden border md:border-2 border-grey rounded-[16px] dark:border-white text-md:flex-row bg-grey-bg dark:bg-grey!">
                                    <div className="promo-card__body flex-1 flex flex-col items-start p-6 ml:p-10 ml:pr-0">
                                        <div className="text-grey dark:text-white mb-3 text-md:mb-4 sm:s-h4 md:l-h4">{t("attend_io_extended")}</div>
                                        <div className="flex-1 flex flex-col justify-between items-start">
                                            <p className="text-grey dark:text-white mb-3 text-md:mb-6 sm:s-cta2 md:l-cta1">{t("join_a_communityled_event_to_learn_and_connect_with_developers_in_your_area")}</p>
                                            <a href="https://gdg.community.dev/ioextended/" rel="noreferrer" target="_blank" className="cta-secondary" onClick={() => analyticsService.trackCTA('Search in your area', 'SpeakerDetail_PromoExtended', 'external_link')}>{t("search_in_your_area")}</a>
                                        </div>
                                    </div>

...
function ExploreContent() {
  ...
  const trackEvent = (action: string, metadata?: any) => {
    analyticsService.trackUI(action, metadata, 'ExplorePage');
  };
  const trackFilter = (globalThis as any).trackFilter || (() => { });
  const trackFilterCategory = (title: string, event: any) => {
    analyticsService.trackUI('filter_category', title, 'ExplorePage');
  };
  useEffect(() => {
  ...
  const handleFilterChange = (id: string, checked: boolean, classification: string) => {
    ...
    setActiveFilters(prev => {
      ...
      const updated = checked ? [...new Set([...current, id])] : current.filter(fid => fid !== id);
      const newFilters = {
        ...prev,
      ...
      router.push(`/explore/${newQuery ? `?q=${newQuery}` : ''}`, {
        scroll: false
      });
      return newFilters;
    });
  ...
  return <div className="w-full min-h-screen bg-white">
    ...
    <main id="content" className="dark:bg-grey-900! dark:text-grey-bg! flex-1">
      <ExploreHero />
      ...
      <div className="page-wrapper flex flex-col program">
        <div id="program-stacks" className="flex flex-col mt-4 md:mt-0 mb-8">
          <div className="flex flex-col justify-between md:flex-row mb-2">
            <span className="sm:s-h4 md:l-h3">{t("what_are_you_building_for")}</span>
            <a className="cta-link-btn mt-4 md:mt-0 p-0! md:self-end" href="/speakers" aria-label={t("view_all_speakers")}>{t("meet_the_wow_speakers")}</a>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-stack-cards-2 ml:grid-cols-stack-cards-4 md:justify-around ml:justify-between gap-y-8 md:gap-3 mt-4 md:mt-10">
            ...
            {[{
              id: 'mobile',
              label: 'Mobile',
              desc: 'Develop for a range of audiences and form factors.',
              color: 'linear-gradient(90deg, #4285F4 -36.98%, #4285F4 22.31%, #34A853 78.95%, #34A853 132.93%)',
              fid: 'b31f8e49-c3ba-438e-a7e1-589b4da62640',
              img: 'io24-stacks-m.svg'
            }, {
              id: 'web',
              label: 'Web',
              desc: 'Create fast, secure sites and apps for the open web.',
              color: 'linear-gradient(270deg, #FFCB32 6.94%, #FFCB32 27.99%, #34A853 73.59%, #34A853 94.64%)',
              fid: '13eba1ea-1c78-47b8-86ef-c7808d2507db',
              img: 'io24-stacks-w.svg'
            }, {
              id: 'ai',
              label: 'ML/AI',
              desc: 'Access cutting-edge AI models and open source tools.',
              color: 'linear-gradient(90deg, #FFCB32 -0.15%, #FFCB32 17.85%, #F46831 52.85%, #EA4335 78.85%, #EA4335 99.85%)',
              fid: '14574666-1892-4a0e-b305-44d6e3f66c56',
              img: 'io24-stacks-a.png'
            }, {
              id: 'cloud',
              label: 'Cloud',
              desc: 'Simplify and scale end-to-end development.',
              color: 'linear-gradient(270.11deg, #5382EB 1.91%, #5382EB 25.69%, #9F6CD4 51.37%, #EA4335 79.9%, #EA4335 97.02%)',
              fid: '622b18d9-a6c2-4d31-b506-a2d58e034186',
              img: 'io24-stacks-c.svg'
            }].map(stack => <a key={stack.id} href={`/explore/?q=${stack.id}`} onClick={e => handleStackClick(e, stack.id, stack.fid)} aria-label={`Jumplink Filter content by ${stack.id} focus.`} className="relative rounded-[20px] md:border-2 dark:md:border-transparent! md:border-transparent min-h-[135px] md:min-h-[297px] hover:border-grey! group dark:hover:border-grey!-bg transition-all">
              <div style={{
        ...
        <MyIOSection isMyIoVisible={isMyIoVisible} isSavedVisible={isSavedVisible} isRecommendedVisible={isRecommendedVisible} setIsMyIoVisible={setIsMyIoVisible} setIsSavedVisible={setIsSavedVisible} setIsRecommendedVisible={setIsRecommendedVisible} />

        <div className="program-content pt-4 md:pt-10">
          <div className="program-content__left">
            <FilterSection categories={FILTER_CATEGORIES} expandedCategories={{
              stack: isFocusExpanded,
              topic: isTopicsExpanded,
              type: isTypeExpanded,
              level: isLevelExpanded
            }} selectedFilters={activeFilters} onCategoryClick={handleCategoryToggle} onFilterChange={handleFilterChange} trackFilter={trackFilter} />
          </div>
            ...
            <div className="h-pill-board">
              <div className={`flex w-full h-auto ${searchQuery ? "pb-8" : ""}`}>
                <div className="z-10 block fixed bottom-4 right-0 md:hidden">
                  <button id="filter-burger" onClick={() => {
                    analyticsService.trackUI('mobile_filter_drawer', 'open', 'ExplorePage');
                    setIsFilterDrawerOpen(true);
                  }} className={`m-filter__chip w-[102px] ring-offset-2 focus:outline-none focus:ring focus:ring-blue dark:focus:ring-blue-dark bg-grey-900 dark:bg-white rounded-full px-3 py-2 ${isScrolled ? "" : "hidden"}`} aria-expanded={isFilterDrawerOpen} aria-label={t("filter_button")} aria-controls="filter-drawer">
                    <img className="dark:hidden" aria-hidden="true" src="/images/filter-icon.svg" width="24" height="24" />
                    <img className="hidden dark:block" aria-hidden="true" src="/images/filter-icon-dark.svg" width="24" height="24" />
                    <span className="font-medium text-white dark:text-grey-900 ml-3">{t("filter")}</span>
                  </button>
                </div>
                {searchQuery && <div className="w-full overflow-x-auto flex gap-1 py-3 px-2 md:flex-wrap items-center">
                  {searchQuery.split(',').filter(Boolean).map(term => {
                    ...
                    return <button key={term} role="presentation" data-chip="true" className="m-filter__chip flex flex-row items-center ring-offset-2 focus:outline-none focus:ring focus:ring-blue dark:focus:ring-blue-dark dark:bg-white bg-grey-900 rounded-lg py-3 px-5 gap-2" aria-label={`remove filter for ${term}`} onClick={() => {
                      ...
                      <img aria-hidden="true" className="dark:block hidden" src="/images/program-close-icon.svg" width="16" height="16" />
                      <img aria-hidden="true" className="block dark:hidden" src="/images/program-close-icon-white.svg" width="16" height="16" />
                    </button>;
                  })}
                  <button className="font-medium underline-link ml-2 dark:text-white" onClick={() => {
                    analyticsService.trackUI('clear_all_filters', 'all', 'ExplorePage');
                    router.push('/explore/', {
                      scroll: false
                    });
                  }} aria-label={t("clear_all")}>{t("clear_all")}</button>
                </div>}
              </div>
            </div>
            {loading ? <div className="w-full flex justify-center py-20">
              <span className="title">{t("loading")}</span>
            </div> : <div role="list" className="w-full grid justify-center grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredSessions.map(session => <SessionCard key={session.id} session={session} trackEvent={trackEvent} />)}
            </div>}
          </div>
        </div>
      </div>
    ...
    <FilterDrawer isOpen={isFilterDrawerOpen} onClose={() => setIsFilterDrawerOpen(false)} categories={FILTER_CATEGORIES} selectedFilters={activeFilters} onFilterChange={handleFilterChange} onClearAll={() => setActiveFilters({
      stack: [],
      topic: [],
      type: [],
      level: []

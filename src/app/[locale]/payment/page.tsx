...
function PaymentPage() {
      ...
      const options = {
        ...
        handler: async (response: any) => {
          ...
          try {
            ...
            analyticsService.trackForm('payment_verify', tierSearch, 'complete', {
              tier: tierSearch
            });
            ...
            const verifyDuration = (Date.now() - startTimeRef.current) / 1000;
            analyticsService.trackCheckoutActivity('payment_verify', tierSearch, 'failed', verifyDuration, {
              error: err.message
            });
            analyticsService.trackForm('payment_verify', 'error_msg', 'error', {
              error: err.message
            });
  ...
  if (earnedBadge) {
    return <div className="w-full min-h-screen bg-[#f8f9fa] dark:bg-grey-900! text-grey-900 dark:text-white flex flex-col transition-colors duration-300">
                ...
                <main className="flex-1 flex flex-col items-center py-0 md:py-16 px-0 md:px-6">
                    <div className="w-full md:max-w-[800px] bg-white dark:bg-grey-800! md:rounded-2xl border-b md:border border-grey-200 dark:border-grey-700 md:shadow-sm overflow-hidden animate-slide-up">
                        <BadgeSuccess badgeName={earnedBadge} onClose={() => {
            setEarnedBadge(null);
            router.push('/explore');
          }} />
                    </div>
  ...
  return <div className="w-full min-h-screen bg-white text-grey-900">
            <Header onRegisterClick={() => {}} />
            <div className="flex flex-col md:flex-row page-wrapper">
                <div ref={arcadeRef} className={`flex-3 p-3 md:mr-4 h-max rounded-[8px] z-20 bg-white/95 backdrop-blur-md self-start group transition-all duration-300 border-b md:border-b-0 border-grey-bg ${isBenefitsVisible ? 'relative md:sticky md:top-0' : 'sticky top-0 w-screen -translate-x-5! p-8'}`}>
                    ...
                    {tiers.find(t => t.name.toLowerCase().includes('arcade')) && <div className={`absolute -top-4 md:top-0 -right-4 md:right-2 w-32 h-32 rotate-12 transition-all duration-300 group-hover:scale-110 group-hover:rotate-12 animate-rotate-in ${isBenefitsVisible ? 'opacity-100' : 'opacity-0 scale-[60%] pointer-events-none'}`}>
                            ...
                            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                                <span className="text-white text-[2.2rem] font-bold leading-none tracking-tighter">
                                    {tiers.find(t => t.name.toLowerCase().includes('arcade'))?.tickets_left || 0}
                                </span>
                                <span className="text-white/80 text-[0.8rem] font-bold uppercase tracking-widest mt-0.5">{t("left")}</span>
                            </div>
                        </div>}

                    <h1 className={`font-medium tracking-tight transition-all duration-300 ${isBenefitsVisible ? 'text-6xl mb-0 mt-8 md:mt-[unset]' : 'text-4xl mb-0 mt-8'}`}>{t("arcade")}</h1>
                    <h3 className={`tracking-tighter transition-all duration-300 overflow-hidden ${isBenefitsVisible ? 'text-4xl mb-4 max-h-[100px] opacity-100' : 'text-xl mb-0 max-h-0 opacity-0'}`}>{t("the_communitys_pass")}</h3>

                    <p className={`max-w-[54ch] mt-2 overflow-hidden transition-all duration-500 ${isBenefitsVisible ? 'max-h-[300px] opacity-100' : 'max-h-0 opacity-0 md:max-h-[300px] md:opacity-100'}`}>{t("get_access_to_the_official_community_arcade_where_you_will_network_learn_connect_and_grow_together_by_playing_games_joining_virtual_sessions_attending_meetups_and_so_much_more")}<br /><br />{t("this_will_help_you_compete_on_different_technical_challenges_make_new_friends_upskill_yourself_in_different_technical_stacks_and_prepare_you_up_for_the_google_for_developers_wow_2026")}</p>
                    <div className={`overflow-hidden transition-all duration-500 ${isBenefitsVisible ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0 md:max-h-[500px] md:opacity-100'}`}>
                        <h4 className="text-xl font-medium tracking-tight my-4">{t("what_will_you_get")}</h4>
              ...
              return <p key={benefit} className={`py-2 px-5 border-2 rounded-full tracking-[-0.015em] font-medium ${benefit == 'Fresh games every other day' || benefit == 'Upto 100% discount on WOW ticket*' ? 'text-white! border-grey-900' : ''}`} style={benefit == 'Fresh games every other day' || benefit == 'Upto 100% discount on WOW ticket*' ? {
                background: 'linear-gradient(90deg, rgb(66,133,244) -36.98%, rgb(66,133,244) 22.31%, rgb(52,168,83) 78.95%, rgb(52,168,83) 132.93%)'
                    ...
                    <div className={`flex items-end w-full transition-all duration-300 ${isBenefitsVisible ? 'pb-8 md:pb-[unset] mt-4 opacity-100' : 'pb-2 mt-2 gap-x-4'}`}>
                        <div className="flex flex-col md:block">
                            <p className={`leading-[1.2] line-through tracking-tighter opacity-30 transition-all text-2xl`}>{t("800")}</p>
                            <p className={`font-bold tracking-tighter transition-all text-3xl`}>{t("350")}<span className={`font-medium opacity-60 ${isBenefitsVisible ? 'text-xl' : 'text-xs'}`}>{t("excl_of_taxes")}</span></p>
                        </div>
                        <div className={`flex ml-auto items-center transition-all ${isBenefitsVisible ? 'flex-col md:flex-row items-end' : 'flex-col gap-2'}`}>
                            {isBenefitsVisible && <a href="/arcade" className={`mt-auto cta-secondary flex items-center rounded-full font-medium transition-all ${isBenefitsVisible ? 'md:mr-4 h-12 md:h-14 text-[14px]! md:text-[20px]!' : 'h-10 px-4 flex items-center justify-center text-[12px]! mx-0!'}`}>{t("know_more")}</a>}
                            <button onClick={() => router.push('/payment?type=arcade')} disabled={isProcessing} className={`nav-cta-btn bg-grey-900! rounded-full bg-google-blue text-white font-medium flex items-center justify-center transition-all ${isBenefitsVisible ? 'mt-2 md:mt-16! px-[20px] py-[12px]! h-12 md:h-14 text-[14px] md:text-[20px]! min-w-[140px]' : 'h-10 px-6 text-[12px]! min-w-[100px] mt-0!'}`}>
                ...
                <div className="pt-4 md:pt-[unset] shrink-0 flex-2 flex flex-col gap-4 md:pl-12 md:border-l-2 md:border-grey-bg">
                    <div className="relative h-[260px] md:h-[240px] mb-9">
                        ...
                        <div className="absolute bg-white dark:bg-grey! w-full h-full rounded-xl border-[1.5px] md:border-2 border-solid border-grey dark:border-grey-bg z-10 overflow-hidden">
                            <div className="flex flex-col items-center justify-center gap-y-1 h-full">
                                <div className="flex flex-row justify-center gap-2 w-full px-6">
                                    <div>
                                        <p className="uppercase font-black tracking-tight text-xs mb-2 border-2 w-max p-1 px-3 -translate-x-2 mb-4 rounded-full border-grey-bg">{t("limited_only")}{tiers.find(t => t.name.toLowerCase().includes('early'))?.tickets_left < 60 && " - Running out"}
                                        </p>
                                        <h3 className="font-bold text-3xl tracking-tight">{t("early_bird_pass")}</h3>
                                        <p className="text-sm max-w-[20ch] line-clamp-4 md:max-w-[28ch] mt-2">{t("get_access_to_all_the_talks_workshops_and_activities_at_the_event_get_access_to_all_the_talks_workshops_and_activities_at_the_event")}</p>
                                    </div>
                                    <div className="flex flex-col items-end text-end ml-auto">
                                        <p className="text-xl line-through tracking-tight opacity-30">{t("1500")}</p>
                                        <p className="text-3xl font-bold tracking-tight">{t("1200")}</p>
                                        <button onClick={() => handlePurchase('Early Bird', 'WOW 2026 - Attendee')} disabled={isProcessing} className={`mt-auto nav-cta-btn bg-grey-900! px-[20px] py-[12px]! h-12 rounded-full bg-google-blue text-white font-medium text-[14px] md:text-[20px] flex items-center justify-center min-w-[120px] ${isProcessing ? 'opacity-70' : ''}`}>
                                            {isProcessing ? '...' : 'Buy now'}
                                        </button>
                                    </div>
                                </div>
                        ...
                        {/* Shadow cards */}
                        <div style={{
            backgroundImage: 'linear-gradient(90deg, rgb(66,133,244) -36.98%, rgb(66,133,244) 22.31%, rgb(52,168,83) 78.95%, rgb(52,168,83) 132.93%)'
            ...
            backgroundImage: 'linear-gradient(90deg, rgb(66,133,244) -36.98%, rgb(66,133,244) 22.31%, rgb(52,168,83) 78.95%, rgb(52,168,83) 132.93%)'
                    ...
                    <div className="border-2 border-grey-bg p-6 flex rounded-[12px] aspect-[2]">
                        <div>
                            <h3 className="font-bold text-3xl mt-2 tracking-tight">{t("regular_pass")}</h3>
                            <p className="text-sm max-w-[20ch] md:max-w-[32ch] mt-2 line-clamp-4">{t("get_access_to_all_the_talks_workshops_and_activities_at_the_event_get_access_to_all_the_talks_workshops_and_activities_at_the_event")}</p>
                        </div>
                        <div className="flex flex-col items-end text-end ml-auto">
                            <p className="text-xl line-through opacity-30">{t("2500")}</p>
                            <p className="text-3xl font-bold">{t("2000")}</p>
                            <button onClick={() => handlePurchase('Regular', 'WOW 2026 - Attendee')} disabled={isProcessing} className={`mt-auto nav-cta-btn bg-grey-900! px-[20px] py-[12px]! h-12 rounded-full bg-google-blue text-white font-medium text-[14px] md:text-[20px] flex items-center justify-center min-w-[120px] ${isProcessing ? 'opacity-70' : ''}`}>
                                {isProcessing ? '...' : 'Buy now'}
                            </button>
                        </div>
                    </div>
                    <div className="border-2 border-grey-bg p-6 flex rounded-[12px] aspect-[2]" aria-disabled={true}>
                        <div>
                            <h3 className="font-bold text-3xl mt-2 tracking-tight">{t("late_bird_pass")}</h3>
                            <p className="text-sm max-w-[20ch] line-clamp-4 md:max-w-[32ch] mt-2">{t("get_access_to_all_the_talks_workshops_and_activities_at_the_event_get_access_to_all_the_talks_workshops_and_activities_at_the_event")}</p>
                        </div>
                        <div className="flex flex-col items-end text-end ml-auto">
                            <p className="text-xl line-through opacity-30 tracking-tight">{t("5000")}</p>
                            <p className="text-3xl font-bold tracking-tight">{t("3500")}</p>
                            <a className="mt-auto cursor-not-allowed! bg-grey-text/30! text-grey-900/60! nav-cta-btn px-[20px] py-[12px]! h-12 rounded-full bg-google-blue text-white font-medium text-[14px] md:text-[20px]">{t("buy_now")}</a>
                        </div>
                    </div>
                    <div className="border-2 border-grey-bg p-6 flex rounded-[12px] aspect-[2]" aria-disabled={true}>
                        <div>
                            <p className="uppercase font-black tracking-tight text-xs mb-2 border-2 w-max p-1 px-3 -translate-x-2 mb-4 rounded-full border-grey-bg">{t("accomodation_included")}</p>
                            <h3 className="font-bold text-3xl mt-2 tracking-tight">{t("all_in_one_pass")}</h3>
                            <p className="text-sm max-w-[20ch] line-clamp-4 md:max-w-[32ch] mt-2">{t("get_access_to_all_the_talks_workshops_and_activities_at_the_event_get_access_to_all_the_talks_workshops_and_activities_at_the_event")}</p>
                        </div>
                        <div className="flex flex-col items-end text-end ml-auto">
                            <p className="text-xl line-through opacity-30 tracking-tight">{t("7500")}</p>
                            <p className="text-3xl font-bold tracking-tight">{t("4500")}</p>
                            <a className="mt-auto cursor-not-allowed! bg-grey-text/30! text-grey-900/60! nav-cta-btn px-[20px] py-[12px]! h-12 rounded-full bg-google-blue text-white font-medium text-[14px] md:text-[20px]">{t("buy_now")}</a>
                        </div>
                    </div>
                </div>
            ...
            {type == 'arcade' && !isTermsAgreed && <div className="z-50 bg-black/40 absolute top-0 h-full w-full flex flex-col items-center justify-center"><div className="flex flex-col items-center text-center h-max bg-white rounded-[12px] overflow-hidden">
                    <div className="h-[240px] bg-grey-bg w-full border-b-2 border-grey-900 justify-center flex items-center flex-col">
                        ...
                        <h2 className="text-2xl font-bold mb-4 whitespace-pre-line absolute m-auto mt-8 py-3 rounded-3xl px-12 bg-grey-bg">{t("terms_conditions")}</h2>
                    </div>
                    <div className="max-w-[484px] p-8">
                        {[{
            label: 'I agree to <a target="_blank" rel="noopener noreferrer" href="/terms">WOW Arcade Terms and Conditions</a> including <a target="_blank" rel="noopener noreferrer" href="/code-of-conduct">Google Community Guidelines</a> and acknowledge that my info will be used in accordance with <a target="_blank" rel="noopener noreferrer" href="/privacy">Privacy Policy</a>',
            id: 'io2024',
            value: terms,
            onChange: (e: any) => setTerms(e.target.checked)
          }, {
            label: 'I acknowledge that puchasing an arcade ticket does not guarantee entry to the event.',
            id: 'io2025',
            value: arcadeAck,
            onChange: (e: any) => setArcadeAck(e.target.checked)
          }].map(item => <div key={item.id} role="listitem">
                                    <div className="filter-box__item">
                                        <input type="checkbox" id={item.id} name={item.id} className="checkbox" checked={item.value} onChange={e => {
                item.onChange(e);
              }} data-analytics-defer="false" data-label={item.label} data-id={item.id} aria-label={item.label} />
                                        <label className="filter-box__option-text text-left faq-entry" htmlFor={item.id} aria-hidden="true">
                                            <div className="">
                    ...
                    __html: item.label
                        ...
                        <button type="button" disabled={!terms || !arcadeAck || isProcessing} onClick={() => {
            if (terms && arcadeAck) {
            ...
            {isProcessing && <div className="fixed inset-0 z-100 bg-white/80 dark:bg-grey-900/80! backdrop-blur-sm flex flex-col items-center justify-center animate-fade-in">
                    <div className="flex flex-col items-center text-center p-8">
                        <div className="w-16 h-16 border-4 border-google-blue border-t-transparent rounded-full animate-spin mb-6"></div>
                        <h2 className="text-3xl font-bold tracking-tight mb-2">{t("verifying_payment")}</h2>
                        <p className="text-grey-600 dark:text-grey-400 max-w-[320px]">{t("hang_tight_were_confirming_your_transaction_with_the_bank_please_dont_refresh_the_page")}</p>
                    </div>
                </div>}
        </div>;
}
export default function SuspensefulPaymentPage() {
  const t = useTranslations();
  return <Suspense fallback={<div className="flex justify-center py-20"><span className="title">{t("loading")}</span></div>}>
            <PaymentPage />
        </Suspense>;
}
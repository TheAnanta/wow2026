...
export default function TeamMemberDetailPage() {
  ...
  if (!member) {
    return <div className="w-full min-h-screen flex flex-col items-center justify-center">
                <h1 className="text-2xl font-medium mb-4">{t("team_member_not_found")}</h1>
                <Link href="/team" className="cta-primary">{t("back_to_team")}</Link>
            </div>;
  }
                ...
                <div className="w-full flex flex-col md:flex-row text-md:h-[407px] bg-grey-bg dark:bg-grey! border-b md:border-b-2 border-grey dark:border-grey-bg">
                    <div className="flex flex-col md:text-left md:justify-center px-4 py-5 w-full md:w-3/5 md:p-10 md:pr-0 dark:text-white z-10">
                        ...
                        <div className="flex flex-row mt-6 md:mt-8">
                            ...
                            <Link className="ml-2 font-medium hover-link" href="/team">{t("see_all_members")}</Link>
                        </div>
                    </div>

                    <div className="flex justify-center md:justify-end items-center w-full md:w-2/5 px-4 py-5 md:p-10">
                        <img className="rounded-full w-48 max-w-md md:w-72 md:h-72 border-2 border-black object-cover" src={member.avatar || '/images/speakers/placeholder.webp'} role="img" aria-hidden="true" loading="lazy" alt={member.name} />
                    </div>
                </div>
                ...
                <div className="page-wrapper px-4 md:px-10">
                    <section className="mt-8 dark:text-grey-200">
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
                            <div>
                                <h2 className="text-2xl md:text-3xl font-medium mb-1">{t("about")}{member.name.split(' ')[0]}</h2>
                                <p className="text-grey-600 dark:text-grey-400">{member.university}</p>
                            </div>
                            {sessions.length > 0 && <Link className="font-medium text-grey underline underline-offset-2 text-[14px] text-md:text-[20px] dark:text-white" href="/explore">{t("see_all_sessions")}</Link>}
                        </div>
                    </section>

                    {/* Sessions Grid (only if applicable) */}
                    {sessions.length > 0 ? <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10 md:mb-14">
                            {sessions.map((session, index) => <div key={session.id} className="group hover:border-[1.5px] border-grey dark:border-grey-bg rounded-xl flex flex-1">
                                    <Link className="flex flex-col relative h-auto w-full" href={`/explore/${session.id}`}>
                                        ...
                                        <div className="relative mt-[-22px] md:mt-[-24px] rounded-xl group-hover:rounded-[10px] border md:border-2 border-solid border-grey dark:border-grey-bg bg-cover h-[44px] md:h-[48px] z-1" style={{
                backgroundImage: GRADIENTS[index % GRADIENTS.length]
              }}></div>
                                        <div className="mt-[-22px] md:mt-[-24px] rounded-xl border md:border-2 border-solid border-grey dark:border-grey-bg bg-cover h-[44px] md:h-[48px]" style={{
                backgroundImage: GRADIENTS[index % GRADIENTS.length]
              }}></div>
                                    </Link>
                                </div>)}
                        </div> : <div className="mb-14 p-10 bg-grey-bg dark:bg-grey! rounded-2xl border-2 border-dashed border-grey-400 dark:border-grey-700 text-center">
                            <p className="text-xl text-grey-600 dark:text-grey-400">{t("this_team_member_is_busy_organizing_the_best_event_ever_stay_tuned_for_more_updates")}</p>
                        </div>}
                                ...
                                <div className="flex-1 flex flex-col overflow-hidden bg-grey-bg dark:bg-grey! border md:border-2 border-grey rounded-[16px] dark:border-white text-md:flex-row">
                                    <div className="promo-card__body flex-1 flex flex-col items-start p-6 ml:p-10 ml:pr-0">
                                        ...
                                        <div className="flex-1 flex flex-col justify-between items-start">
                                            ...
                                            <a href="https://developers.google.com/events" rel="noreferrer" target="_blank" className="cta-secondary">{t("find_an_event")}</a>
                                        </div>
                                    </div>
                                ...
                                <div className="flex-1 flex flex-col overflow-hidden border md:border-2 border-grey rounded-[16px] dark:border-white text-md:flex-row bg-grey-bg dark:bg-grey!">
                                    <div className="promo-card__body flex-1 flex flex-col items-start p-6 ml:p-10 ml:pr-0">
                                        ...
                                        <div className="flex-1 flex flex-col justify-between items-start">
                                            ...
                                            <a href="https://gdg.community.dev/ioextended/" rel="noreferrer" target="_blank" className="cta-secondary">{t("search_in_your_area")}</a>
                                        </div>
                                    </div>

    ...
    <main id="content" className="dark:bg-grey-900! flex-1">
      ...
      <div className="w-full flex flex-col md:flex-row text-md:h-[407px] overflow-hidden bg-grey-bg dark:bg-grey! border-b-[1px] md:border-b-2 border-grey dark:border-grey-bg">
        <div className="flex flex-col md:text-left md:justify-center px-4 py-5 w-full md:w-2/5 md:p-10 md:pr-0 dark:text-white z-10 ">
          <h1 className="font-medium mb-4 sm:s-h2 md:l-h1 text-md:-mr-40">{t("community")}</h1>
          <p className="font-medium sm:s-h6 md:l-h6 mb-4">{t("meet_developers_discover_local_groups_and_build_your_global_network")}</p>
          <div>
            <div className="h-cta-button">
              <button type="button" className="cta-primary block" onClick={() => router.push('/register')}>
                <span>{t("register_for_gdg_wow")}</span>
              </button>
            </div>
          </div>
        </div>
        <div className="flex justify-end items-end w-full md:w-3/5 ">
          <img className="hidden md:inline-block h-full object-cover object-left dark:hidden " src="/images/io24-community-hero.svg" role="img" aria-hidden="true" height={407} />
          <img className="hidden dark:md:inline-block h-full object-cover object-left " src="/images/io24-community-hero-dark.svg" role="img" aria-hidden="true" height={407} />
          <img className="block md:hidden dark:hidden " src="/images/io24-community-hero-mobile.svg" role="img" aria-hidden="true" />
          <img className="hidden dark:inline-block dark:md:hidden " src="/images/io24-community-hero-mobile-dark.svg" role="img" aria-hidden="true" />
        </div>
      </div>
      ...
      <div className="page-wrapper flex flex-col">
        ...
        <div className="flex flex-col align-center items-stretch justify-center gap-[32px] md:flex-row mb-12 md:mb-[40px]">
          ...
          <div className="flex w-full md:w-1/2">
            <div className="flex-1 flex flex-col overflow-hidden bg-grey-bg dark:bg-grey! border-[1px] md:border-2 border-grey rounded-[16px] dark:border-white text-md:flex-row">
              <div className="promo-card__body flex-1 flex flex-col items-start p-6 ml:p-10 ml:pr-0">
                <div className="text-grey dark:text-white mb-3 text-md:mb-4 sm:s-h4 md:l-h4">{t("join_us_at_wow")}</div>
                <div className="flex-1 flex flex-col justify-between items-start">
                  <p className="text-grey dark:text-white mb-3 text-md:mb-6 sm:s-cta1 md:l-cta1">{t("explore_network_and_get_handson_with_the_latest_products")}</p>
                  <a className="cta-secondary" href="/explore" target="_blank" rel="noreferrer">{t("find_an_session")}</a>
                </div>
              </div>
          ...
          <div className="flex w-full md:w-1/2">
            <div className="flex-1 flex flex-col overflow-hidden border-[1px] md:border-2 border-grey rounded-[16px] overflow-hidden dark:border-white text-md:flex-row bg-grey-bg dark:bg-grey!">
              <div className="promo-card__body flex-1 flex flex-col items-start p-6 ml:p-10 ml:pr-0">
                <div className="text-grey dark:text-white mb-3 text-md:mb-4 sm:s-h4 md:l-h4">{t("attend_gdgoc")}</div>
                <div className="flex-1 flex flex-col justify-between items-start">
                  <p className="text-grey dark:text-white mb-3 text-md:mb-6 sm:s-cta1 md:l-cta1">{t("join_a_communityled_event_to_learn_and_connect_with_developers_in_your_area")}</p>
                  <a className="cta-secondary" href="https://gdg.community.dev/" target="_blank" rel="noreferrer">{t("search_in_your_area")}</a>
                </div>
              </div>
        ...
        <div className="flex flex-col text-black text-center md:text-left dark:text-grey-bg">
          <span className="font-medium mb-4 text-md:mb-6 sm:s-h4 md:l-h2">{t("join_a_community_group")}</span>
          <p className="font-medium text-grey mb-2 text-md:mb-6 sm:s-p2 md:l-cta1 dark:text-grey-200">{t("engage_with_developers_and_technology_experts_to_collaborate_and_build_your_network")}</p>
        </div>
            ...
            <div className="md:flex-1 w-full">
              <div className="flex relative w-full md:max-w-[1234px]">
                <div className="flex items-center border-l-2 border-r-2 border-t-2 border-black dark:text-grey rounded-[100px] w-full h-[46px] md:h-14 bg-grey-bg dark:bg-white border-b-2">
                  ...
                  <input type="text" aria-label={t("enter_your_citytown")} placeholder={t("enter_your_citytown")} value={searchQuery} onChange={handleSearch} className="focus:outline-none focus:ring-0 border-0 bg-transparent p-0 text-lg font-medium w-full placeholder:text-ellipsis placeholder-shown:text-ellipsis placeholder-grey" />
                </div>
              </div>
            </div>
            <div className="md:mt-[14px]">
              <p>
                <a className="cta-link-btn inline-block mt-[20px] see-all-button" href="https://developers.google.com/community" target="_blank" rel="noreferrer">{t("see_all_communities")}</a>
              </p>
            </div>
          </div>
        </div>
        ...
        {/* Search Results */}
        {searchResults && searchResults.length > 0 ? <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-16 px-4">
          {searchResults.map(result => <div key={result.id} className="flex items-center p-4 border border-[#000000] dark:border-white rounded-lg bg-white dark:bg-grey! py-6">
            <div className="w-12 h-12 border border-[#000000] dark:border-white rounded-[4px] mr-5 flex-shrink-0" style={{
              background: 'linear-gradient(135deg, #34A853, #4285F4)'
            }} />
            <div className="flex flex-col">
              <div className="font-medium mb-1 text-lg dark:text-white uppercase leading-none">{result.name}</div>
              <div className="text-sm underline cursor-pointer dark:text-white font-medium" onClick={() => {
                console.log('Joined chapter', result.id);
                analyticsService.trackCTA(`join_chapter_${result.id}`, 'CommunityPage');
              }}>{t("join_chapter")}</div>
            </div>
          </div>)}
        </div> : isSearching ? <div className="mb-16 text-center py-10 dark:text-white">{t("searching")}</div> : null}
          ...
          <div className="w-full">
            <div className="flex flex-col align-center ml:flex-row h-full bg-grey-bg dark:bg-grey! border-[1px] md:border-2 border-grey rounded-[16px] overflow-hidden dark:border-white">
              <div className="flex flex-col items-start p-6 pb-0 ml:w-1/2 ml:p-10 ml:pr-0">
                <span className="text-grey dark:text-white mb-3 text-md:mb-4 sm:s-h4 md:l-h4 ml:-mr-24">{t("plan_your_wow")}</span>
                <p className="text-grey dark:text-white mb-3 text-md:mb-6 sm:s-cta1 md:l-cta1 ml:-mr-24">{t("visit_my_wow_for_saved_content_and_recommendations_based_on_your_personal_interests")}</p>
                <a href="/explore#my-wow" className="cta-secondary">{t("get_started")}</a>
              </div>
